...
import com.example.act1pmdm_jj.ui.theme.Elementos
import com.example.act1pmdm_jj.ui.theme.Gray
...
fun Actividad1() {
    ...
    //Creao un texto variable que recueldde el valor que tinene que tener el text de cada boton
    var textoButton by rememberSaveable { mutableStateOf("") }
    ...
    Column(
        ...
        if (showLoading) {
            ...
            //genero el spacio una vez sea visible las cargas
            Spacer(Modifier.padding(vertical = 30.dp))
        }else{
            textoButton ="Cargar perfil"
        }

        Button(
            onClick = { showLoading = !showLoading }
        ) {
            Text(text = textoButton )
        }
    }
}
...
fun Actividad3() {
    Column(
        ...
        var barraProgreso by rememberSaveable { mutableStateOf( 0f)}

        LinearProgressIndicator(progress = barraProgreso)
        ...
        Row(
            ...
            //En cada onClick compruebo con un if si tiene su valor minimo y maximo.
            Button(onClick = {
                if (barraProgreso<1){barraProgreso += 0.1f}
            }) {
            ...
            Button(onClick = {
                if (barraProgreso>0){barraProgreso -= 0.1f}
            }) {
                Text(text = "Reducir")
            }
        }
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun Actividad5(value:String,onCambio:(String)->Unit) {
    ...
    Box(
        modifier = Modifier
            .fillMaxSize()
            .clickable { focusManager.clearFocus() }, // Clic en cualquier parte de la pantalla
        contentAlignment = Alignment.Center
    ) {
        OutlinedTextField(
            modifier = Modifier
